...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const updated = DBManager.updateOrderStatus(parseInt((await params).id), status, riderInfo);
  if (!updated) return NextResponse.json({ error: 'অর্ডার পাওয়া যায়নি' }, { status: 404 });
